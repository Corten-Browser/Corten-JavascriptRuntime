...
try:
    ...
    try:
        ...
        from orchestration.markdown_parser import MarkdownParser, CodeBlock
    except ImportError:
        # Fallback if markdown_parser not available
        MarkdownParser = None
        CodeBlock = None

# Import test quality checker (v0.5.0)
try:
    from test_quality_checker import TestQualityChecker
except ImportError:
    TestQualityChecker = None
...
class CompletionVerifier:
    ...
    def _check_test_quality(self, component_path: Path) -> CheckResult:
        ...
        try:
            checker = TestQualityChecker(component_path)
