#!/usr/bin/env python3
"""
Self-check to ensure orchestration system is properly installed.

This module verifies that all required files are present and properly
configured. It can be run standalone or imported by other tools.

Usage:
    python orchestration/self_check.py
    python orchestration/self_check.py --verbose
    python orchestration/self_check.py --fix
...
class OrchestratonSelfCheck:
    ...
    def check_required_files(self) -> CheckResult:
        ...
        required_files = [
            'orchestration/git_retry.py',
            'orchestration/git_status.py',
            'orchestration/templates.py',
            'orchestration/orchestration-config.json',
            'CLAUDE.md',
            '.claude/commands/orchestrate.md',
            '.claude/commands/orchestrate-full.md',
        ]
    ...
    def run_all_checks(self) -> Tuple[int, int, int]:
        ...
        print(f"{Colors.BLUE}  Orchestration System Self-Check{Colors.NC}")
        print(f"{Colors.BLUE}═══════════════════════════════════════════════════════════{Colors.NC}\n")
    ...
    def print_installation_help(self):
        ...
        print(f"\n{Colors.YELLOW}To install orchestration system:{Colors.NC}")
        print("  git clone https://github.com/org/claude-orchestration.git")
        print("  claude-orchestration/scripts/install.sh .")
        print(f"\n{Colors.YELLOW}Or with fix mode:{Colors.NC}")
        print("  python orchestration/self_check.py --fix")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Verify orchestration system installation'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
